...
## ------------------
## par(mfrow=c(2,1))
## plot(dat$X$synced_time[1:5000],dat$X$hd[1:5000], xlim=c(0,100), type="l")
## plot(Traw.data[1:5000], HDraw.data[1:5000], xlim=c(0,100), type="l")
...
## training set
## 
## dim(coords.trap)[1] == length(HD.data); length(HD.data) == length(T.data); length(T.data) == length(index.CV)
## tmp <- Yposraw %>% dplyr::filter(index.CV %in% train.index) %>% dplyr::select(sp, Ti, HDi, index.CV) %>%
##     ## mutate(T=)
## ## unnest(cols=c(sp, Ti, HDi))
## Yposraw %>% dplyr::filter(index.CV %in% train.index) %>% select(ep, index.CV) %>% unnest(cols=ep)
## coords.trap <-
...
## dT            <- c(do.call("c", (Yposraw %>% dplyr::filter(index.CV %in% train.index))$Ti)) ## c(diff(T.data))
## dGamma.test   <- c(do.call("c", (Yposraw %>% dplyr::filter(!(index.CV %in% train.index)))$Li))
## dT.test       <- c(do.call("c", (Yposraw %>% dplyr::filter(!(index.CV %in% train.index)))$Ti))## c(diff(T.data.test))
## dGamma.raw    <- c(do.call("c", Yposraw$Li))
## dT.raw        <- c(do.call("c", Yposraw$Ti)) ## c(diff(T.data))
...
df.join <- data.frame(coords.x1 = mesh$loc[,1], coords.x2= mesh$loc[,2])
W.ipoints.M0 <- left_join(df.join, data.frame(coords.x1 = mesh$loc[W.ipoints.M0@i+1,1],
...
df.W.M0.test <- df.prism.M0.test %>% group_by(index.CV) %>% nest() %>% 
    mutate(W.M0.vector = map(data, function(x){
        ...
        W.ipoints.M0.test <- left_join(W.ipoints.M0, data.frame(coords.x1 = mesh$loc[W.ipoints.M0.test@i+1,1],
                                        coords.x2 = mesh$loc[W.ipoints.M0.test@i+1,2],
                                        weight=W.ipoints.M0.test@x), by = c("coords.x1", "coords.x2") ) %>%
...
df.W.M1.test <- df.prism.M1.test %>% group_by(index.CV) %>% nest() %>% 
    mutate(W.M1.vector = map(data, function(x){
        ...
        ## IP 29/06/2022
        W.ipoints.M1.test <- left_join(W.ipoints.M1, data.frame(
...
##                                                                                               weights=weights.domain@data[,1],
##                                                                                               block=rep(1, nrow(weights.domain@coords))), nrow=1)))
## A.temporal.field_constr <- inla.spde.make.A(mesh=mesh1d, loc=weights.domain1d$x,
##                                            weights=weights.domain1d@weight,
##                                            block=rep(1, length(weights.domain1d$x)))
...
if(FALSE){
    ...
    se.score.M2.space.time      <- (pred.means.M2.space.time - obs.firings)^2
    se.score.M1.space.direction <- (pred.means.M1.space.direction - obs.firings)^2
    se.score.M1.space.direction2 <- (pred.means.M1.space.direction2 - obs.firings)^2
    ## Dawid-Sebastiani score
    ds.score.M0                 <- (se.score.M0 / pred.vars.M0) + log(pred.vars.M0)
    ds.score.M2.space.time      <- (se.score.M2.space.time / pred.vars.M2.space.time) + log(pred.vars.M2.space.time)
    ds.score.M1.space.direction <- (se.score.M1.space.direction / pred.vars.M1.space.direction) + log(pred.vars.M1.space.direction)
    ds.score.M1.space.direction2 <- (se.score.M1.space.direction2 / pred.vars.M1.space.direction2) + log(pred.vars.M1.space.direction2)
    ## -----------------------------------------------------------------------------
}
