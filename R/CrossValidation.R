...
pred.mean.var <- function(mod, newdata=NULL, weights.mat, n.samples=10^3){
  ...
  GRF <- lapply(1:n.samples, function(i) post.samp[[i]]$spde2)     # simulated GRFs 
...
## ----------------------------------------------
# beta.M0           <- fit.space.2.train$summary.fixed$mean  # model fit.space.2.train doesn't exist
...
#beta.M0           <- fit.space$summary.fixed$mean
#x.M0              <- fit.space$summary.random$spde2$mean
# length(x.M0) # 1209

## ----------------------------------------------------------------
## Model based estimates (M0) of number of events for each connected
## line segment in the test set
## NOTE: weights_test$df.W.M0$W.ipoints.M0 is a length of length 14
## ----------------------------------------------------------------
...
## ## check: is this matrix needed for model fitting?
## Ypos.sldf <- SpatialLinesDataFrame(sl = SpatialLines(lapply(as.list(1:nrow(Ypos)),
##                                                             function(k) Lines(list(Line(cbind(c(Ypos$coords[k,1],
##                                                                                                 Ypos$coords.lead[k,1]),
##                                                                                               c(Ypos$coords[k,2],
