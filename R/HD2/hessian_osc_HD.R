if(FALSE){
    hessian.objective_osc_HD <- function(par, theta, data, mesh, A, W, tW, Aobs, order.HD, method="biased"){
        ## input:
        ...
        ## --------------------------------------------------
        hess.beta  <- -as.numeric(exp(beta))*(tW%*%expx) 
        ## 
        hess.Xbeta <- Matrix( -as.numeric(exp(beta))*as.numeric(W*expx) )
        ## 
        hess <- Matrix::forceSymmetric(rBind( cBind(-hess.X + Q.X, -hess.Xbeta), cBind(-t(hess.Xbeta), -hess.beta + Q.beta) ))
...
hessian.objective_osc_HD <- function(par, theta, data, mesh, A, tA, W, tW, Aobs, order.HD, method="biased"){
    ## input:
    ##        par: named list containing vector of parameters X and scalar parameter beta
    ##        theta: fixed hyperprior parameters!! (revisit selection)
    ##        data: list of Y and Ypos (list(Y, Ypos))
    ...
    hess.Xbeta <- Matrix(0, nrow=length(X), ncol=length(beta))
    ## --------------------------------------------------
    hess.X     <- -as.numeric(exp(beta))* (tA %*% (Diagonal(length(W), as.numeric(W*expAx)) %*% A))
    hess.beta  <- -as.numeric(exp(beta))*sum(W*expAx) 
    hess.Xbeta <- Matrix( -as.numeric(exp(beta))*tA%*%(Diagonal(length(W), W)%*%expAx))    
    hess <- Matrix::forceSymmetric(rBind( cBind(-hess.X + Q.X, -hess.Xbeta), cBind(-t(hess.Xbeta), -hess.beta + Q.beta) ))
    ## hessian_diag_and_upper_triangle(beta=as.numeric(beta), p,
    ##                                 L=as.matrix(L), A=as.matrix(A),
    ##                                 expAx=as.matrix(expAx))
