objective_osc_HD <- function(par, theta, data, mesh, A, Aobs, W, tW=tW, order.HD, method="biased") {
    ## input:
    ...
    X    <- par$X
    beta <- par$beta
    out  <-  -prior.betaX_osc_HD(X=X, beta=beta, theta=theta, mesh = mesh, order.HD = order.HD) - #!!!! need to change priorXbeta_osc
        pp.llik(data=data, X=X, mesh=mesh, beta=beta, A=A, Aobs=Aobs, W=W, tW=tW, method=method)
    return(out)
}
