pthetapc.prop.marg.post_osc_HD <- function(par.theta, hyperpar, data, X, beta, mesh, L, A, W, Aobs, type="biased", acc=1e-7, print.verbose=FALSE)
{
    ## input:
...
##     lp.theta    <- prior.theta_osc(rho=(2*pi)*(exp(par.theta[1])/(1+exp(par.theta[1]))), sigma=exp(par.theta[2]),
##                                     rhoL=hyperpar$rhoL, sigmaU=hyperpar$sigmaU,
##                                    alpha1=hyperpar$alpha1, alpha2=hyperpar$alpha2,
##                                    lg=TRUE)
##     obj <- as.numeric(-(lp.theta + lp.Xbeta + llik - lp.G))
