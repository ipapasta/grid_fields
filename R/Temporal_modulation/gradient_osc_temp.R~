grad.objective_osc_temp <- function(par, theta,  data,  mesh, mesh1d, A, Atilde, Aobs, Atildeobs, W){
    ## input:
    ...
    grad.Xbeta <- -rBind(grad.beta, grad.X, grad.Z) + rBind(Q.beta%*%beta, Q.X%*%X, Q.Z%*%Z ) # grad beta, X, Z
    return(grad.Xbeta)    
}        
