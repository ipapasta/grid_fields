hessian.objective_osc_temp<- function(par, theta,  data,  mesh, mesh1d, A, Atilde, Aobs, Atildeobs, W, W.input.to.sparseMatrix){
    ## input:
    ...
    Q.X     <- osc.precision(theta=theta.X, mesh=mesh) 
    Q.Z     <- temp.precision(theta=theta.Z, mesh=mesh1d)
    if(FALSE){
        gradbeta.gradbeta_T  <- Matrix(0, nrow=length(beta), ncol=length(beta))
        gradX.gradX_T        <- Matrix(0, nrow=p, ncol=p)
        gradZ.gradZ_T        <- Matrix(0, nrow=ptilde, ncol=ptilde)
        gradbeta.gradX_T     <- Matrix(0, nrow=length(beta), ncol=p)
        gradbeta.gradZ_T     <- Matrix(0, nrow=length(beta), ncol=ptilde)
        gradZ.gradX_T        <- Matrix(0, nrow=ptilde, ncol=p)
    }
    ## --------------------------------------------------
    gradbeta.gradbeta_T  <- -as.numeric(exp(beta))*(t(expZ) %*% (W%*%expX)) 
    ##------
    bX.T <- (t(tW %*% expZ) * (t(expX)))
    system.time(bX.T <- (t(tW %*% expZ) * (t(expX))))
    bZ.T <- (t(W %*% expX) * (t(expZ)))
    system.time(bZ.T <- (t(W %*% expX) * (t(expZ))))
    ## -----
    ...
    ## ----
    o <- W.input.to.sparseMatrix$value * Z[W.input.to.sparseMatrix$knot] * X[W.input.to.sparseMatrix$v]
    ## ----
    gradZ.gradX_T  <-  -as.numeric(expbeta) * sparseMatrix(i=W.input.to.sparse.matrix$knot, j=W.input.to.sparse.matrix$v, x=o) 
    ## 
