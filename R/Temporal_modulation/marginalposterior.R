...
pthetapc.prop.marg.post_osc_temp<- function(par.theta, hyperpar, data, X, Z, beta, mesh, mesh1d, A, Atilde, Aobs, Atildeobs, W,
                                            W.input.to.sparseMatrix, acc=1e-7, print.verbose=FALSE)
{
    ## input:
    ...
    ## ---------------------------------------
    ## starting values    
    grad     <- grad.objective_osc_temp(par,theta=par.theta,data=data,mesh=mesh,mesh1d=mesh1d,A=A,Atilde=Atilde,Aobs=Aobs,Atildeobs=Atildeobs,W=W)
    hess     <- Matrix(hessian.objective_osc_temp(par,theta=par.theta,data=data,mesh=mesh,mesh1d=,mesh1d,A=A,Atilde=Atilde,Aobs=Aobs,Atildeobs=Atildeobs,W=W,
                                                  W.input.to.sparseMatrix=W.input.to.sparseMatrix),sparse=TRUE)
    fval.p   <- fval.c <- as.numeric(objective_osc_temp(par=par,theta=par.theta,data=data,mesh=mesh,mesh1d=mesh1d,A=A,Atilde=Atilde,Aobs=Aobs,Atildeobs=Atildeobs,W=W))
    ...
    while(TRUE){    #!!!{
        ...
        ## Newton move
        ## 
        grad  <- Matrix(grad.objective_osc_temp(par,theta=par.theta,data=data,mesh=mesh,mesh1d=mesh1d,A=A,Atilde=Atilde,Aobs=Aobs,Atildeobs=Atildeobs,W=W),ncol=1)
        hess  <- Matrix(hessian.objective_osc_temp(par, theta=par.theta,data=data,mesh=mesh,mesh1d=,mesh1d,A=A,Atilde=Atilde,Aobs=Aobs,Atildeobs=Atildeobs,
                                                   W=W,W.input.to.sparseMatrix=W.input.to.sparseMatrix), sparse=TRUE) #!
        ## 
        d       <- -Matrix::solve(hess, grad)
    ...
    LbetaXZ.given.thetay    <- (Matrix::Cholesky(hess) %>% (Matrix::expand))$L    
    logdetQbetaXZ.given.thetay <- 2*sum(log(diag(LbetaXZ.given.thetay)))
    lp.G     <- .5*logdetQbetaXZ.given.thetay
    lp.betaXZ <- prior.betaXZ_osc_temp(beta=par$beta, X=par$X, Z=par$Z, theta=c(par.theta),  mesh=mesh, mesh1d=mesh1d)
    ...
    ## (data=data, X=par$X, mesh=mesh, L=L, beta=par$beta, A=A,  Aobs=Aobs, W=W, method=method)
    ## output
    ## double assignment
    betaest <<- par$beta
    Xest    <<- par$X
    ...
    print(paste("rho is:", exp(par.theta[1]), " sigma is:", exp(par.theta[2]),
                ...
                "rho_temporal is: ", exp(par.theta[4]), "sigma_temporal is: ", exp(par.theta[5])))   
    lp.theta    <- prior.theta_osc_temp(rho=exp(par.theta[1]), sigma=exp(par.theta[2]), phi = (1-exp(-par.theta[3]))/(1+exp(-par.theta[3])),
                                        ...
                                        rho.temp=exp(par.theta[4]), sigma.temp=exp(par.theta[5]),
                                        rho.tempL=hyperpar$rho.tempL, sigma.tempU=hyperpar$sigma.tempU,
                                        alphaT1=hyperpar$alphaT1, alphaT2=hyperpar$alphaT2,
                                        lg=TRUE)
    ...
    save(betaest, Xest, Zest, par.theta, gradest, Hessianest, obj, file = paste0("/data/ipapasta/Xbeta_temp_mod", counter, ".RData"))
    ##     }
    ## }
