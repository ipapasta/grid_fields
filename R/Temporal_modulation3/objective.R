objective_osc_temp <- function(par, theta, data, mesh, mesh.theta, mesh1d, A, Atilde, Aobs, Atildeobs, W) {
    ## input:
    ...
    X    <- par$X
    Z    <- par$Z
    out  <-  - prior.betaXZ_osc_temp(beta=beta, X=X, Z=Z, theta=theta, mesh = mesh, mesh.theta, mesh1d=mesh1d) -
