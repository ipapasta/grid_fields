prior.theta_osc_temp <- function(rho, sigma, phi,
                                 rho.hd, sigma.hd,
                                 rho.temp,sigma.temp)  # hyperparameters of temporal process
{
    ## input:
    ...
    ## base model has infinite range and 0 variance
    ## ---------------
    ## spatial process
    ## ---------------
    sigmaU <- sigmaU.hd <- sigmaU.temp <- 1
    alpha2 <- 1e-1
    alphahd2 <- alphaT2 <- 1e-4
    sigmaLN <- 0.5
    murho   <- 25
    ...
    ## lsigma.hd  <- log(l2.hd) - l2.hd*sigma.hd
    ## ---------------
    ## temporal
    ## ---------------
    murho.temp   <- 7
    ...
    ## PRIOR
    outl    <- lpphi.sp + lrho.hd + lsigma.hd+
        lrho.sp + lsigma.sp + 
        lrho.temp + lsigma.temp
    ## outl <- 0
    return(outl)
}

if(FALSE){
    ## prior for phi parameter
    lpphi   <- prior.phi_osc(seq(-.9,.9,len=100), a=2, b=10, lg=TRUE)
    plot(seq(-.9,.9,len=100), exp(lpphi))
    ## prior for range parameter
    sigmaLN <- 0.3
    murho   <- 25
    plot(seq(5, 100, len=100), exp(dlnorm(seq(5, 100, len=100), log(murho), sigmaLN, log=TRUE)))
    ## prior for sigma
    alpha2     <- 1e-1
    sigmaU     <- 1
    l2tilde <- -log(alpha2)/sigmaU
    plot(seq(1e-3, 10, len=100),  exp(log(l2tilde) - l2tilde*seq(1e-3, 10, len=100)))
    plot(seq(1e-3, 10, len=100),  (log(l2tilde) - l2tilde*seq(1e-3, 10, len=100)))
    ## 
    logrhoL    <- log(10)                                                                                                                               
    logsigmaU  <- log(1) #                                                                                                                              
    alpha1     <- .001                                                                                                                                  
    ...
    alphaT1     <- .001                                                                                                                                 
    alphaT2     <- 1e-6
    rhoo   <- seq(10, 80, len= 200)       
    sigmaa <- seq(1e-6, 1, len= 200)       
    rhoolg   <- seq(1e-1, 1000, len= 200)       
    sigmaalg <- seq(0.001, 1.5, len= 200)       
    mat    <- matrix(nrow=200, ncol=200)
    matlg    <- matrix(nrow=200, ncol=200)    
}

if(FALSE){
    prior.theta_osc_temp <- function(rho, sigma, phi, rhoL, sigmaU, alpha1, alpha2, # hyperparameters of spatial component
                                     rho.temp, sigma.temp, rho.tempL, sigma.tempU, alphaT1, alpha2T, # hyperparameters of temporal component
                                     lg=TRUE)
    {
        ## input:
        ##       range: range parameter of spatial field
        ...
        l2tilde <- -log(alpha2)/sigmaU
        l1Ttilde <- -log(alphaT1)*rho.tempL
        l2Ttilde <- -log(alphaT2)/sigma.tempU
        lpphi   <- prior.phi_osc(phi, a=2, b=30, lg=TRUE)
...
prior.theta_osc <- function(rho, sigma, phi, rhoL, sigmaU, alpha1, alpha2, lg=TRUE)
{
    ...
    l1tilde <- -log(alpha1)*rhoL
    l2tilde <- -log(alpha2)/sigmaU
    lpphi   <- prior.phi_osc(phi, a=2, b=50, lg=TRUE)
...
prior.theta_osc_fixedphi <- function(rho, sigma, phi, rhoL, sigmaU, alpha1, alpha2, lg=TRUE)
{
    ## input:
    ##       range: range parameter of spatial field
    ##       sigma: sigma parameter of spatial field
    ...
    ## l1tilde <- -log(alpha1)*rhoL
    ## l2tilde <- -log(alpha2)/sigmaU
    ## lpphi   <- -log(2) + dbeta((1+phi)/2, shape1 = 1/2, shape2 = 4/1 , log=TRUE)
    ## outl    <- log(l1tilde) + log(l2tilde) - l1tilde/rho - l2tilde*sigma  ## + lpphi
    ## outl    <- log(l1tilde) + log(l2tilde) - l1tilde/rho - l2tilde*sigma  ## + lpphi
    ## if(phi > 0) return(-Inf)
    ## outl    <- 0
...
if(FALSE)
{
    ...
    filled.contour(rhoo, sigmaa, mat, xlab="rho", ylab="st deviation", main="joint prior",
                   ylim=c(0, 1), nlevels=50, color.palette = palette,
