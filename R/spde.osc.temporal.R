...
df.prism.M2.space              <- df.prism.M1_M2 %>% dplyr::select(-c(val.M1, val.M2.space.direction.time)) %>% unnest(cols=c(val.M2.space.time))
...
df.W.M2.space <- rbind(df.prism.M2.space %>% mutate(group=tk, dGamma.lag=0) %>%
