...
##     hd=X$hd, time=X$synced_time,
##     ## coords=I(lapply(as.list(apply(cbind(X$position_x, X$position_y),1, as.list)), unlist))) %>%
##     coords=cbind(X$position_x, X$position_y)) %>%
##     mutate(coords.lead.1 = lead(coords.1),
##            coords.lead.2 = lead(coords.2),
##            time.lead = lead(X$synced_time),
##            hd.lead = lead(X$hd)) %>%
##     head(-1)

## Xraw.sldf <- SpatialLinesDataFrame(sl   = SpatialLines(lapply(as.list(1:nrow(X.raw)),
##                                                               function(k) Lines(list(Line(cbind(c(X.raw$coords.1[k],
##                                                                                                   X.raw$coords.lead.1[k]),
##                                                                                                 c(X.raw$coords.2[k],
##                                                                                                   X.raw$coords.lead.2[k])))), ID=k))),
##                                    data = X.raw %>% dplyr::select(-c(coords.1, coords.2, coords.lead.1, coords.lead.2)))

## ips.inlabru.raw <- ipoints(samplers = Xraw.sldf, domain = mesh)
...
## weights.domain <- ipoints(domain=mesh, samplers=SPls.Omega)
## weights.full.domain <- ipoints(domain=mesh)
## locs <- weights.domain@coords
## rownames(locs) <- NULL
...
## A.full.spatial.field_constr <- matrix(weights.full.domain$weight, nrow=1)
...
## ## ggplot() +
## ##     gg(mesh)+
## ##     gg(weights.domain, aes(size = weight)) + coord_fixed() +
## ##     theme_classic()


## ## ggplot() +
## ##     gg(mesh)+
## ##     gg(ips.inlabru, aes(size = weight)) +
## ##     gg(spikes, color="red") + 
## ##     ## +
...
## ## ggplot() +
## ##     gg(mesh)+
## ##     gg(ips.inlabru.raw, aes(size = weight)) +
## ##     gg(spikes, color="red") + 
## ##     ## +
## ##     coord_fixed(xlim=c(0,100), ylim=c(0,100))
...
## sum(fit.space$summary.random$f$mean * A.spatial.field_constr)
