...
Additionally to the spatial and the meshes, a temporal mesh is also
required for models that add a temporal effect in the variation of
spikes. The temporal mesh is constructed by thinning the grid of times
that are obtained from the newly derived line segments. Below we
