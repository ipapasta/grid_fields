...
subregion of the domain of interest on the left panel below. Each
newly derived line segment is marked by the start and end of an arrow
whilst the raw segments are defined by adjacent blue dots. Similarly
for the head direction on the right panel below, each newly derived
line segment is given by adjacent black circles whilst the raw
segments are given by adjacent blue dots.
#+begin_src R :results output code :exports code :session *R:grid_fields*  :tangle yes
  par(mfrow=c(1,2))
